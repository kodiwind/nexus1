...
def getCacheSize():
	...
	if INCLUDEVIDEO == 'true':
		...
		'''else:
			if INCLUDEATHENA == 'true':    files.append(os.path.join(ADDOND, 'plugin.video.athena', 'cache.db'))
			if INCLUDECOVEN == 'true':    files.append(os.path.join(ADDOND, 'plugin.video.covenant', 'cache.db'))
			if INCLUDEPARKER == 'true':    files.append(os.path.join(ADDOND, 'plugin.video.parker', 'cache.db'))
			if INCLUDEINJUSTICE == 'true':  files.append(os.path.join(ADDOND, 'plugin.video.injustice', 'cache.db'))
			if INCLUDESUBZERO == 'true':    files.append(os.path.join(ADDOND, 'plugin.video.subzero', 'database.db'))
			if INCLUDEMORBIUS == 'true':    files.append(os.path.join(ADDOND, 'plugin.video.morbius', 'cache.db'))
